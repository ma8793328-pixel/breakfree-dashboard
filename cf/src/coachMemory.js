...
export async function loadMemory(env, userId, habitId) {
  const row = await env.DB.prepare(
    'SELECT topics, follow_ups, mood_trend, last_topic, updated_at FROM coach_memories WHERE user_id = ? AND habit_id = ?'
  ).bind(userId, habitId).first();
  return memoryRowToState(row);
}

export async function saveMemory(env, userId, habitId, message) {
  try {
    const topics = extractTopics(message);
    const existing = await loadMemory(env, userId, habitId);
    ...
    ).bind(userId, habitId, JSON.stringify(mergedTopics), JSON.stringify(combinedFollowUps), moodTrend, lastTopic);
  } catch (e) {
